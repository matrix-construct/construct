...
mr_server(struct MsgBuf *msgbuf_p, client::client &client, client::client &source, int parc, const char *parv[])
{
	...
	{
		sendto_realops_snomask(sno::GENERAL, is_remote_connect(&client) ? L_NETWIDE : L_ALL,
	...
	if(!DoesTS(&client))
	{
		sendto_realops_snomask(sno::GENERAL, L_ALL, "Link %s dropped, non-TS server",
				     client.name);
...
ms_server(struct MsgBuf *msgbuf_p, client::client &client, client::client &source, int parc, const char *parv[])
{
	...
	if(!hlined)
	{
		...
		sendto_realops_snomask(sno::GENERAL, L_ALL, "Non-Hub link %s introduced %s.",
				     client.name, name);
